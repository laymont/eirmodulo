...
if(isset($_GET['f1']) and isset($_GET['f2'])){
	...
	$cadenaSQL = sprintf("SELECT lineas.nombre AS linea, buques.nombre AS buque, viajes.viaje, tequipos.tipo, inventario.contenedor, inventario.fdb, inventario.fdm, inventario.frd, inventario.fdespims, inventario.eir_r, inventario.fact, inventario.`status`, inventario.condicion, inventario.precinto, inventario.bl, patios.patio, consignatario.nombre AS consig, inventario.obs FROM inventario, lineas, buques, viajes, tequipos, patios, consignatario WHERE inventario.`delete` = 0 AND inventario.linea = lineas.id AND inventario.buque = buques.id AND inventario.viaje = viajes.id AND inventario.tcont = tequipos.id AND inventario.patio = patios.id AND inventario.`consignatario` = consignatario.id AND inventario.fdespims BETWEEN '%s' AND '%s' ORDER BY inventario.frd ASC;",$inicio,$fin);
	...
	$sql = sprintf("SELECT tequipos.tipo, count(inventario.tcont) AS cantidad FROM inventario, tequipos WHERE inventario.tcont = tequipos.id AND inventario.`delete` = 0 AND tequipos.tipo LIKE '2%%' AND inventario.fdespims BETWEEN '%s' AND '%s' GROUP BY inventario.tcont ORDER BY tequipos.tipo ASC;",$inicio,$fin);
	$recaps20->Consulta($sql);
	
	$recaps40 = new DBMySQL();
	$recaps40->Datosconexion(UDB,PDB,USERDB);
	$sql = sprintf("SELECT tequipos.tipo, count(inventario.tcont) AS cantidad FROM inventario, tequipos WHERE inventario.tcont = tequipos.id AND inventario.`delete` = 0 AND tequipos.tipo LIKE '4%%' AND inventario.fdespims BETWEEN '%s' AND '%s' GROUP BY inventario.tcont ORDER BY tequipos.tipo ASC;",$inicio,$fin);
	$recaps40->Consulta($sql);
	
//exportar a excel
header('Content-type: application/vnd.ms-excel');
header("Content-Disposition: attachment; filename=egresosGral.xls");
...
<?php if($recaps40->Num_resultados > 0){ ?>
    ...
    <?php } ?>
<table align="center" class="listado">
  <caption>
  ...
  <tr>
    ...
    <td scope="strd"><?php echo $inventario->Filas['fdespims']; ?></td>
    ...
    <td scope="num" data-align="right"><span id="da">
            <?php FechaDif($inventario->Filas['frd'],$inventario->Filas['fdespims']); ?>
            </span></td>
          <td scope="num" data-align="right"><span id="dp">
            <?php FechaDif($inventario->Filas['fdb'],$inventario->Filas['fdespims']);?>
            </span></td>
  </tr>
